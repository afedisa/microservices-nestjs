...
export class CompanyMemberService {
  constructor(
    @InjectRepository(CompanyMemberEntity, Database.PRIMARY)
  ...
  async findAll(
    ...
    const members = await this.companyMemberRepository.findBy({
  ...
  async findByUser(
    ...
    const member = await this.companyMemberRepository.findOneBy({
      user: { id: user.id },
    });
    return {
      state: !!member,
