...
export class CompanyInvitationService {
  constructor(
    @InjectRepository(CompanyInvitationEntity, Database.PRIMARY)
    private companyInvitationRepository: Repository<CompanyInvitationEntity>,
  ...
  async create({
    company,
  }: CreateCompanyInvitationDto): Promise<
    IServiceResponse<CompanyInvitationEntity>
  > {
    let result;
  ...
  async findAll({
    ...
    IServiceResponse<IPagination<CompanyInvitationEntity>>
  > {
    ...
    const invitesCount = await this.companyInvitationRepository.count({
      where,
    });
    return {
      state: true,
  ...
  async use(
    ...
    if (finded) {
      ...
      if (updated) {
        const { data: company } = await this.companyService.findById(
          invite.companyId,
        );
        const { state: isJoined, data: companyMember } =
          await this.companyMemberService.create(company, user);
        if (isJoined) {
          result = invite;
        }
      }
  ...
  async update(
    id: string,
    updateDto: Partial<CompanyInvitationEntity>,
  ): Promise<IServiceResponse<CompanyInvitationEntity>> {
    let result;
  ...
  async validateMaxInviteCount(
    ...
    const invites = await this.companyInvitationRepository.findBy({
