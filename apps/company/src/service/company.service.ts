...
export class CompanyService {
  constructor(
    @InjectRepository(CompanyEntity, Database.PRIMARY)
  ...
  async create(
    ...
    let result;
    const { state: canCreate } = await this.validateCompanyCountLimitation(
      user.id,
    );
    if (canCreate) {
  ...
  async update(
    id: string,
    updateDto: Partial<CompanyEntity>,
  ): Promise<IServiceResponse<CompanyEntity>> {
    let result;
  ...
  async validateCompanyCountLimitation(
    userId: string,
  ): Promise<IServiceResponse<boolean>> {
    const ownedCompanies = await this.companyRepository.findBy({
      ownerId: userId,
    });
