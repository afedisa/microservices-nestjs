...
import {
  ...
  OtpDriverType,
  OtpTemplate,
} from 'apps/otp/src/interface/otp.interface';
...
export class AuthService {
  ...
  async start({
    phone,
  }: StartAuthDto): Promise<IServiceResponse<AuthRequestEntity>> {
    ...
    if (existRequest) {
      ...
      // const { state: otpSended } = await firstValueFrom(
      //   this.otpClient.send<IServiceResponse<boolean>, IOtpRequest>(
  ...
  async validate({
    phone,
    code,
  }: ValidateAuthDto): Promise<IServiceResponse<AuthRequestEntity>> {
