...
export class CompanyGatewayController {
  ...
  async getCompanies(
    ...
    const { state, data: companies } = await firstValueFrom(
      this.companyClient.send<
        IServiceResponse<IPagination<CompanyEntity>>,
        FindCompaniesDto
      >(COMPANY_MESSAGE_PATTERNS.FIND_ALL, findDto),
    );
    return {
      state,
      data: companies,
  ...
  async createCompany(
    ...
    const { state, data: company } = await firstValueFrom(
      this.companyClient.send<
        IServiceResponse<CompanyEntity>,
        { createDto: CreateCompanyDto; user: UserEntity }
