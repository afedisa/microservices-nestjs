...
import {
  StorageFileBucket,
  StorageFileDriverType,
  StorageFileType,
} from 'apps/storage/src/interface/storage-file.interface';
  ...
  async updateProfile(
    ...
    const { state, data } = await firstValueFrom(
      this.userService.send<
        ...
        { id: string; updateDto: UpdateUserDto }
  ...
  async uploadProfileAvatar(
    ...
    const { state: storageState, data: storageData } = await firstValueFrom(
      this.storageService.send<
        IServiceResponse<StorageFileEntity>,
        { createDto: CreateStorageFileDto; user: UserEntity }
    ...
    if (storageState) {
      const { state: updateState } = await firstValueFrom(
        this.userService.send<
          IServiceResponse<UpdateUserDto>,
          { id: string; updateDto: UpdateUserDto }
        >(USER_MESSAGE_PATTERNS.UPDATE, {
          id: user.id,
          updateDto: {
