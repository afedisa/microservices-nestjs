...
export class VehicleGatewayController {
  ...
  async getVehicles(
    @Query() findDto: FindVehiclesDto,
  ): Promise<IGatewayResponse<IPagination<VehicleEntity>>> {
    const { state, data: vehicles } = await firstValueFrom(
      this.vehicleClient.send<
        IServiceResponse<IPagination<VehicleEntity>>,
        FindVehiclesDto
      >(VEHICLE_MESSAGE_PATTERNS.FIND_ALL, findDto),
    );
    return {
      state,
      data: vehicles,
