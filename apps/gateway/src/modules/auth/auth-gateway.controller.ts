...
export class AuthGatewayController {
  ...
  async startAuthentication(
    ...
    const { state, message } = await firstValueFrom(
      this.authClient.send<IServiceResponse<AuthRequestEntity>>(
        AUTH_MESSAGE_PATTERNS.START,
        startDto,
      ),
    );
  ...
  async validateAuthentication(
    ...
    const { state, data, message } = await firstValueFrom(
      this.authClient.send<IServiceResponse<AuthRequestEntity>>(
        AUTH_MESSAGE_PATTERNS.VALIDATE,
        validateDto,
      ),
    );
    if (state && data) {
      ...
      let { data: user } = await firstValueFrom(
        this.userClient.send<IServiceResponse<UserEntity>, string>(
          USER_MESSAGE_PATTERNS.FIND_BY_PHONE,
          phone,
        ),
      );
      ...
      const {
        ...
        this.authClient.send<IServiceResponse<IJwtToken>, IJwtPayload>(
          AUTH_MESSAGE_PATTERNS.CREATE_TOKEN,
